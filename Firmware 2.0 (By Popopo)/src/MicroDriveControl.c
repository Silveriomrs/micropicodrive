...
void check_ui_notifications(mdactivestatus_t previousState, bool fromGap) {
    switch(previousState) {
                                ...
                                //image and fill the buffers with the next sector
        {
            ...
            if(fromGap) readEvt.arg = secondBufferSet ? 0 : 1;
            else readEvt.arg = secondBufferSet ? 1 : 0;
        ...
        case MDA_WRITE_SECTOR:  //We were writting a sector to the ULA, notify the UI that it needs to fill the buffers with the next sector
        {
            ...
            //If we are called from a gap routine the buffer set has already been changed
            //but if we are being called from the deselection method then the bufferset has not been changed.
            if(fromGap) writeEvt.arg = secondBufferSet ? 0 : 1;
            else writeEvt.arg = secondBufferSet ? 1 : 0;
...
bool common_gap_code(uint8_t** selectedTrack1Buffer, uint8_t** selectedTrack2Buffer, bool forRead) {
    ...
    if(forRead) sleep_PIO_write();
    else begin_PIO_write_gap();
    begin_PIO_read_gap();
    ...
    if(isHeader) {
        ...
        //Choose the correct buffers Header
        if(secondBufferSet) {
        ...
        //Choose the correct buffers Sector
        if(secondBufferSet) {
...
void begin_write_gap() {
    ...
    if(isHeader) activeStatus = MDA_WRITE_HEADER_GAP;
    else activeStatus = MDA_WRITE_SECTOR_GAP;
...
void begin_read_gap() {
    ...
    mdactivestatus_t previousState = activeStatus;
    //Set our new active status
    if(isHeader) activeStatus = MDA_READ_HEADER_GAP;
    else activeStatus = MDA_READ_SECTOR_GAP;
    //Clear any pending data in the FIFOs
...
void write_gap_alarm(uint alarm_num) {
    ...
    if(activeStatus != MDA_WRITE_HEADER_GAP && activeStatus != MDA_WRITE_SECTOR_GAP) return;
    //Create the event
...
static inline void abort_shifter_alarm() { hardware_alarm_cancel(SHIFTER_ALARM); }

//Function to cancel the write gap alarm
static inline void abort_write_gap_alarm() { hardware_alarm_cancel(WRITE_GAP_ALARM);}
    ...
    switch(mdevt->event) {
        case MDE_SHIFT_CHANGED:
            ...
            if(mdevt->args == 0) {
                ...
                if(mdStatus == MDS_SELECTED) deselect_md(); //Deselect us
            } else if (mdStatus == MDS_DESELECTED) {
        ...
        case MDE_SELECT_TIMEOUT_EXPIRED:
            ...
            if(mdStatus == MDS_DESELECTED) select_md();
            break;
        case MDE_MD_STATUS_CHANGED:
            ...
            if(mdStatus != MDS_SELECTED || !isCartridgeInserted) return;
        ...
        case MDE_DMA_WRITE_IRQ:
            ...
            if(mdevt->args == 1) track1DMAFired = true;
            else track2DMAFired = true;
        ...
        case MDE_CHECK_WRITE_FINISH:
            ...
            } else begin_write_gap();         //Start a new gap
            break;
        default:
            break;
    }
 ...
 * @param uint track1DMA or track2DMA
*/
void track_write_irq(uint track){
     //Create irq event
    mdcontrolevent_t writeEvent;
    //Clear the interrupt flag
    if(track == track1DMA) {
        dma_channel_acknowledge_irq0(track); //For track1DMA
        writeEvent.args = 1;
    } else {
        dma_channel_acknowledge_irq1(track); //For track2DMA
        writeEvent.args = 2; 
    }
...
// //The PIO machine has finished sending data to the ULA.
void track1_write_irq() { track_write_irq(track1DMA); }

// //The PIO machine has finished sending data to the ULA.
void track2_write_irq() { track_write_irq(track2DMA); }
...
void status_irq() {
    ...
    if(newMdStatus == MDL_INVALID) return;
...
/**
 * Function (Auxiliary) to set a dma channel configuration (aka. track) with its working values.
 * @param *track dma channel to setup. In this version they are defined by fields.
 * @param tdma dma to be asigned to the track.
 * @param pio the PIO instance. 
 * @param sm state machine index (0..3).
 * @param is_tx is a TX set?. True for sending data to the state machine, false for receiving data from the state machine.
 */
void setTrack(dma_channel_config *track, uint tdma, PIO pio, uint sm, bool is_tx){
    *track = dma_channel_get_default_config(tdma);
    channel_config_set_dreq(track, pio_get_dreq(pio, sm, is_tx));
...
void init_DMAs() {
    ...
    setTrack(&track1ReadConfig, track1DMA, pio0, sm_read_head_1, false);

    //configure the track2 read config
    setTrack(&track2ReadConfig, track2DMA, pio0, sm_read_head_2, false);
